...
describe("Functions: campaign deletion job", () => {
  ...
  it("deletes a campaign and its full dependent tree across chunked calls", async () => {
  ...
  it("rejects starting a deletion job when a character has no usable Recovery Code", async () => {
    const dmUid = await signInTestUser();
    const campaignRef = adminDb.collection("campaigns").doc();
    await campaignRef.set({ dmId: dmUid, name: "Test Campaign", memberIds: [] });
