...
describe("Functions: repairSessionSummaries", () => {
  ...
  it("rebuilds member-safe summaries without copying DM notes", async () => {
    ...
    const repair = httpsCallable<{ campaignId: string }, { repairedCount: number }>(
      getTestFunctions(),
  ...
  it("rejects a signed-in stranger without creating summaries", async () => {
    const dmUid = await signInTestUser();
    const campaignRef = adminDb.collection("campaigns").doc();
