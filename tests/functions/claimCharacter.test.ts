...
describe("Functions: claimCharacter", () => {
  ...
  it("claims an unclaimed character and atomically rotates the consumed code", async () => {
  ...
  it("rejects reusing a code after its successful claim consumed it", async () => {
    const dmUid = await signInTestUser();
    const campaignRef = adminDb.collection("campaigns").doc();
    const characterRef = campaignRef.collection("characters").doc();
    await campaignRef.set({ dmId: dmUid, name: "Test Campaign", memberIds: [] });
  ...
  it("only lets one of two truly concurrent claims for the same character succeed", async () => {
    const dmUid = await signInTestUser();
    const campaignRef = adminDb.collection("campaigns").doc();
    const characterRef = campaignRef.collection("characters").doc();
    await campaignRef.set({ dmId: dmUid, name: "Test Campaign", memberIds: [] });
