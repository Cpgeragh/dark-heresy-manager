...
describe("Functions: registerRecoveryCode", () => {
  ...
  it("generates a Recovery Code for the DM's own campaign character, over the real emulator", async () => {
  ...
  it("logs a history entry when rotating an existing code, but not on first registration", async () => {
    const dmUid = await signInTestUser();
    const campaignRef = adminDb.collection("campaigns").doc();
    const characterRef = campaignRef.collection("characters").doc();
    await campaignRef.set({ dmId: dmUid, name: "Test Campaign" });
