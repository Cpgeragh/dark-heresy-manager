...
describe("Functions: reconcileCharacterSpentXp", () => {
  ...
  it(
    "lets the DM correct a stale spent total without touching total or ranks",
    async () => {
      ...
      const reconcileCharacterSpentXp = httpsCallable(getTestFunctions(), "reconcileCharacterSpentXp");
  ...
  it(
    "does not write when the spent value is already correct",
    async () => {
      const dmUid = await signInTestUser();
      const campaignRef = adminDb.collection("campaigns").doc();
      const characterRef = campaignRef.collection("characters").doc();
  ...
  it(
    "rejects an invalid spent value",
    async () => {
      const dmUid = await signInTestUser();
      const campaignRef = adminDb.collection("campaigns").doc();
      const characterRef = campaignRef.collection("characters").doc();
