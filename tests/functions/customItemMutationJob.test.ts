...
describe("Functions: custom item mutation job", () => {
  ...
  it("publish-and-update propagates a draft version to every holding character", async () => {
  ...
  it("archive-and-remove archives the item and strips every character's copy", async () => {
    const dmUid = await signInTestUser();
    const campaignRef = adminDb.collection("campaigns").doc();
    await campaignRef.set({ dmId: dmUid, name: "Test Campaign", memberIds: [] });
  ...
  it("rejects starting a mutation job for a caller who is not the campaign DM", async () => {
    const dmUid = await signInTestUser();
    const campaignRef = adminDb.collection("campaigns").doc();
    await campaignRef.set({ dmId: dmUid, name: "Test Campaign", memberIds: [] });
    const itemRef = campaignRef.collection("customItems").doc();
