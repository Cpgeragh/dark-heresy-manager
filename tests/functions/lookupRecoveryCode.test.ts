...
describe("Functions: lookupRecoveryCode", () => {
  ...
  it("resolves a real registered code to a minimal, correctly-owned preview", async () => {
  ...
  it("returns not-found for a well-formed code that was never registered", async () => {
  ...
  it("exhausts the real per-code rate limit after repeated lookups, simulating a brute-force guess sequence", async () => {
    await signInTestUser();
