...
describe("getNextTalentCost", () => {
  it("returns the real cost for an unlocked, unowned talent", () => {
    expect(getNextTalentCost("Guardsman", "Conscript", "sound-constitution", undefined, [])).toBe(
      100
    );
  });
  ...
  it("returns undefined once every currently-unlocked slot at this rank is owned", () => {
    const owned = [
      entry("sound-constitution"),
      entry("sound-constitution"),
      entry("sound-constitution"),
    ];
    expect(
      getNextTalentCost("Guardsman", "Conscript", "sound-constitution", undefined, owned)
    ).toBeUndefined();
  });
  ...
  it("buys cheapest-unlocked-slot-first across multiple ranks", () => {
    ...
    expect(
      getNextTalentCost("Guardsman", "Veteran", "sound-constitution", undefined, ownedFive)
    ).toBe(100);
    const ownedSix = Array.from({ length: 6 }, () => entry("sound-constitution"));
    expect(
      getNextTalentCost("Guardsman", "Veteran", "sound-constitution", undefined, ownedSix)
    ).toBe(200);
  });

  it("returns undefined for a talent never on the career's table at all", () => {
    expect(
      getNextTalentCost("Guardsman", "Veteran", "psy-rating-1", undefined, [])
    ).toBeUndefined();
  });
...
describe("getNextTalentCost, trait entries", () => {
  it("resolves a real trait cost too, not just talent entries", () => {
    expect(
      getNextTalentCost("Adept", "Sage Logister", "unnatural-characteristic", "Intelligence", [])
    ).toBe(500);
  });
});
...
describe("isTalentMaxedAtCurrentRank", () => {
  it("is false for a talent not on the career's table at all", () => {
    expect(
      isTalentMaxedAtCurrentRank("Guardsman", "Conscript", "psy-rating-1", undefined, [])
    ).toBe(false);
  });

  it("is false while unlocked slots remain unbought", () => {
    expect(
      isTalentMaxedAtCurrentRank("Guardsman", "Conscript", "sound-constitution", undefined, [])
    ).toBe(false);
  });
  ...
  it("is true once every currently-unlocked slot is owned", () => {
    ...
    expect(
      isTalentMaxedAtCurrentRank("Guardsman", "Conscript", "sound-constitution", undefined, owned)
    ).toBe(true);
  });
});
