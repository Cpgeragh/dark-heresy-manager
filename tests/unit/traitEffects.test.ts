...
function trait(
  ...
  over: Partial<TalentEntry> = {}
): TalentEntry {
  return {
    uid: `${talentId}:${specialisation ?? "plain"}`,
    talentId,
    name: talentId,
    specialisation,
    ...over,
...
describe("Trait cross-page effects", () => {
  it("derives selected Homeworld, Talent-granted and Career Traits without saving duplicate entries", () => {
    const talents = block({
      ...
      talents: [
        ...
        trait("the-flesh-is-weak", undefined, { uid: "flesh-2" }),
      ],
    });
  ...
  it("applies Trait characteristic changes and suppresses Fit For Purpose for Labourer Build", () => {
    ...
    expect(
      getTraitCharacteristicModifierSources(talents, "t", "Adept").map((source) => source.amount)
    ).toEqual([10, 3]);
    ...
    const soulBound = block({
      traits: [
        trait("soul-bound", undefined, {
          acquisition: {
            trait: {
              soulBound: {
                entity: "The Emperor",
  ...
  it("labels selected-Homeworld Skill rules as Homeworld while preserving other origins", () => {
    ...
    expect(getTraitSkillEffects(forge, commonTech)).toEqual(
      expect.objectContaining({
        countsAsBasic: true,
        sources: [expect.objectContaining({ name: "Forge World", type: "Homeworld" })],
      })
    );
    expect(getTraitSkillEffects(forge, machineCult)).toEqual(
      expect.objectContaining({
        countsAsBasic: true,
        sources: [expect.objectContaining({ name: "Forge World", type: "Homeworld" })],
      })
    ...
    const blank = block({
      traits: [
        trait("blank-slate", undefined, {
          acquisition: {
            trait: { blankSlateSkillIds: [lore.id, "trade-copyist", "forbidden-inquisition"] },
          },
        }),
      ],
    });
    const blankEffects = getTraitSkillEffects(blank, lore);
    expect(blankEffects).toEqual(
      expect.objectContaining({
    ...
    const talents = block({
      ...
      traits: [
        trait("soul-bound", undefined, {
          acquisition: {
            trait: {
              soulBound: { entity: "The Emperor", consequence: "insanity", rolledValue: 4 },
            },
          },
        }),
      ],
    });
    expect(getTraitInsanityModifierSources(talents)).toEqual(
      expect.arrayContaining([
  ...
  it("counts a manually-purchased Sanctioned Psyker only once when the same career also derives it", () => {
    const manual = trait("sanctioned-psyker", undefined, {
      ...
      acquisition: {
        trait: {
          sanctioning: { resultId: "hunted", resultName: "Hunted", rolledValue: 1, ageIncrease: 5 },
        },
      },
    });
    const talents = block({
  ...
  it("derives Blind as a read-only consequence of Soul-bound blindness", () => {
    const talents = block({
      traits: [
        trait("soul-bound", undefined, {
          acquisition: {
            trait: { soulBound: { entity: "The Emperor", consequence: "blindness" } },
          },
        }),
      ],
    });
    expect(getDerivedTraitEntries(talents)).toEqual(
      expect.arrayContaining([
  ...
  it("derives Homeworld Talents, Weapon Training and Wary initiative from recorded choices", () => {
    ...
    expect(getTraitGrantedTalentSpecs(noble)).toEqual(
      expect.arrayContaining([
    ...
    const schola = block({
      ...
      homeworldTraitChoices: { basicWeaponGroup: "Las", pistolWeaponGroup: "SP" },
    });
    expect(getTraitGrantedWeaponTrainingIds(schola)).toEqual([
      "basic-las",
      "melee-primitive",
      "pistol-sp",
    ]);
    ...
    expect(
      getActiveTraitEntries(schola).some((entry) => entry.talentId === "homeworld-skill-at-arms")
    ).toBe(true);
  });
});
