...
describe("Firestore Rules: Advanced Query Operations", () => {
  it("a DM can query their own campaigns with an additional where clause", async () => {
    ...
    await createCampaign(env, `query-c2-${timestamp}`, "dm-query-1", {
      name: "Beta",
      system: "Dark Heresy",
    });
    await createCampaign(env, `query-c3-${timestamp}`, "dm-query-1", {
      name: "Gamma",
      system: "Dark Heresy",
    });
  ...
  it("unauthenticated users cannot query campaigns", async () => {
    ...
    await expect(
      anonDb.collection("campaigns").where("name", "==", "Alpha").get()
    ).rejects.toThrow();
  });
  ...
  it("an unrelated authenticated user cannot query campaigns without a membership-proving where clause", async () => {
    ...
    await expect(
      dbAs(env, "player-bystander").collection("campaigns").where("name", "==", "Alpha").get()
    ).rejects.toThrow();
  });
