...
| Message history           |                                                                      100 returned per page | Bounds every live/history read                                                                                                        |
...
- `CampaignsProvider` coordinates the DM and member views of active campaigns, and starts during the splash screen, concurrently with the remaining account-resolution work;
- `useCampaignCustomItems` merges bounded, server-filtered category/status queries, accepts a small category set so a multi-category tab does not open duplicate subscriptions, and waits until the campaign role (DM or player) is known before it starts;
- `useCharacterData` subscribes only to the character document; claim history has its own deliberately enabled hook.

Expensive listeners follow the visible UI lifecycle. `MessageDrawer` mounts its thread only while open, DM Inbox mounts messages only for the expanded thread, and both Campaign Overview and the character-sheet Admin tab start claim history only while the DM has explicitly opened History. The character sheet's Armour, Drugs, Archeotech, Gear, Weapons and Cybernetics tabs load their own custom items without a dedicated tab-level loading screen; the character sheet's own combined loading covers this. Campaign Overview's DM-facing character list and the player-facing party roster both derive their small name/owner/claim-status rows from `characterSummaries`, a restricted per-character record kept in sync by every character write and every ownership transition (claim, release, force-assign, force-release); only a character's own page reads the full document.
...
Firestore rules are the authoritative boundary for client access. In addition to ownership checks, current rules reject unexpected document fields and validate the stored shape, primitive types and relevant size limits for user accounts, public profiles, campaign metadata, characters, custom items and versions, recovery records, device-link records, DM-only full sessions, member-safe session summaries, claim logs, message-thread summaries and individual messages. Recovery indexes, account lifecycle records, identity secrets and device links are writable only by protected Cloud Functions. Deep polymorphic character and custom-item content is additionally checked by the exact client validator because Firestore's 1,000-expression evaluation ceiling prevents recursively reproducing the complete TypeScript schema in one rule evaluation; the rules still enforce exact top-level document fields, critical nested discriminators, collection sizes and Firestore's own document-size ceiling. Client validation improves feedback but does not replace the rule boundary.

Every account has a permanent generated account id that is separate from Firebase Auth device ids. The first device and every later device access that account through `userLinks`; there is no primary/secondary device and no hidden account underneath a connection. A recovery code entered during onboarding performs one protected, rate-limited connection action. Settings can disconnect any one device without deleting the account, while disconnecting the final device requires an additional recovery-code warning. The live link and profile subscriptions open the shared account without moving data between device identities. Settings shows the account's connected devices as a live, access-controlled list (`listLinkedDevices.ts`), ready before the app itself becomes usable. Linking a new device is bounded by a server-enforced, transaction-safe count (`deviceCount.ts`): an account already at or over the limit stays fully usable but cannot link another device, with a clear rejection, until it drops back under the limit.

Character creation writes the character document directly with an empty Recovery Code (`recoveryCode: ''`); registering the real code, and everything about recovery lookup and claiming, is handled entirely by protected Cloud Functions through the Admin SDK (see `functions/src/operations/registerRecoveryCode.ts`, `lookupRecoveryCode.ts` and `claimCharacter.ts`). `recoveryIndex/{code}` denies all client read and write access; only the Admin SDK ever touches it, keyed by an HMAC-derived identifier rather than the raw code. A character's own stored code is never included in a bulk or list read; the DM's character list and a player's own character card each reveal it on demand, one character at a time, through the protected `revealRecoveryCode` callable, which checks the caller is the campaign DM or the character's owning player before returning it.

Message owners may update a thread summary only through the send-message state transition: the last message and timestamp change and the unread count rises by exactly one. A DM summary write must be one of three shapes: a reply changing only preview/timestamp, a non-zero unread count changing to zero, or a clear operation producing an empty preview/timestamp and zero unread count. Preview text is capped at 500 characters. Every connected device identifies as the same permanent account, while unrelated account identities are rejected.
...
Custom-item drafts may be created only by the campaign DM or a user present in the campaign membership array, with every connected device acting as the same permanent account. Item and version documents have exact top-level schemas, bounded names, text, maps, arrays and version numbers. Only the DM may publish, archive, restore or permanently remove library definitions.
...
- account creation, completion, connection, setup cancellation and device disconnection are transactional; retrying a lost account-creation response returns the same provisional account and code instead of creating another account;
- disconnecting a device changes only its link and onboarding state, while account deletion releases owned characters and removes every connected-device link before deleting the account identity.
...
| Active player campaigns                  | `campaigns`: `memberIds CONTAINS`, `archivedAt ASC`   | Supports the Dashboard's membership `array-contains` query while excluding archived campaigns |
...
| `identityService`       | Create accounts and reveal, rotate or clear account recovery codes                                               |
