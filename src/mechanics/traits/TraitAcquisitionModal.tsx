...
import { notesForSanctioning } from "./traitEffects";
...
function requiredRollRange(resultId: string): { min: number; max: number; label: string } | null {
  if (resultId === "reconstructed-skull")
    return { min: 5, max: 50, label: "Throne Gelt gained (5d10)" };
  if (resultId === "hunted") return { min: 1, max: 10, label: "Insanity Points gained (1d10)" };
  if (["unlovely-memories", "the-horror"].includes(resultId))
    return { min: 1, max: 5, label: "Insanity Points gained (1d5)" };
  return null;
}
...
export function TraitAcquisitionModal({
  ...
  const eligibleBlankSkills = useMemo(
    () =>
      DEFAULT_SKILLS.filter((skill) =>
        ["Common Lore", "Forbidden Lore", "Scholastic Lore", "Trade"].includes(skill.category)
      ),
    []
  );
  ...
  if (picker === "soul-consequence") {
    return (
      <OptionPickerScreen
        title="Binding Consequence"
        options={SOUL_CONSEQUENCES}
  ...
  if (picker === "skin-kind") {
    return (
      <OptionPickerScreen
        title={`Rank ${skinRank} Benefit`}
        options={[
          { value: "new", label: "Gain a new Common cybernetic" },
          { value: "upgrade", label: "Upgrade an existing cybernetic to Good" },
        ]}
        selected={skinKind}
        onSelect={(value) => {
          setSkinKind(value as "new" | "upgrade");
          setSkinCyberneticId("");
          setSkinLocation("");
          setPicker(null);
        }}
  ...
  if (picker === "skin-cybernetic") {
    return (
      <OptionPickerScreen
        title="Common Cybernetic"
        options={CYBERNETICS_REFERENCE.map((item) => ({ value: item.id, label: item.name }))}
        selected={skinCyberneticId}
        onSelect={(value) => {
          setSkinCyberneticId(value);
          setSkinLocation("");
          setPicker(null);
        }}
        onClose={() => setPicker(null)}
  ...
  if (picker === "skin-upgrade") {
    return (
      <OptionPickerScreen
        title="Cybernetic to Upgrade"
        options={upgradeCandidates.map((item) => ({
          value: item.id,
          label: `${item.name} (${item.craftsmanship})`,
        }))}
        selected={skinCyberneticId}
        onSelect={(value) => {
          setSkinCyberneticId(value);
  ...
  if (picker === "skin-location" && selectedCybernetic?.requiresLocation) {
    const options: PickerOption[] =
      selectedCybernetic.requiresLocation === "arm"
        ? [
    ...
    return (
      <OptionPickerScreen
        title="Body Location"
        options={options}
        selected={skinLocation}
        onSelect={(value) => {
          setSkinLocation(value as ArmourLocationKey);
          setPicker(null);
        }}
        onClose={() => setPicker(null)}
      />
            ...
            <Button
              ...
              onClick={() =>
                onComplete({
                  entry: {
                    ...entry,
                    notes: `Imprinted Skills: ${blankSkills.map((id) => DEFAULT_SKILLS.find((skill) => skill.id === id)?.name).join(", ")}.`,
                    acquisition: {
                      ...entry.acquisition,
                      trait: { blankSlateSkillIds: blankSkills },
                    },
                  },
                })
              }
  ...
  if (trait.acquisition === "soul-bound") {
    ...
    const canSubmit =
      ...
      !!consequence &&
      (!rollRequired || (Number.isInteger(roll) && roll >= 1 && roll <= 10)) &&
      (consequence !== "characteristic" || !!characteristic) &&
      (consequence !== "mutation" || mutationName.trim().length > 0);
    return (
      <PickerModal
        title="Soul-bound Acquisition"
        query=""
        onQueryChange={() => undefined}
        onClose={onClose}
        hideSearch
      ...
      >
        <PickerBody>
          <div className="space-y-3">
            ...
            <PickerField
              id="soul-bound-consequence"
              label="Permanent consequence"
              value={SOUL_CONSEQUENCES.find((option) => option.value === consequence)?.label ?? ""}
              placeholder="Choose consequence…"
              required
              onClick={() => setPicker("soul-consequence")}
            />
            {consequence === "characteristic" && (
              <PickerField
                id="soul-bound-characteristic"
                label="Characteristic"
                value={
                  CHARACTERISTICS.find((option) => option.value === characteristic)?.label ?? ""
                }
                placeholder="Choose Characteristic…"
                required
                onClick={() => setPicker("characteristic")}
              />
            )}
            {rollRequired && (
              <div>
                ...
                <input
                  ...
                  value={rolledValue}
                  onChange={(event) =>
                    setRolledValue(sanitizePositiveIntegerInput(event.target.value))
                  }
                  placeholder="1–10"
                  className={`${editableInputClass(true)} mt-0.5`}
                />
            ...
            <Button
              ...
              onClick={() =>
                onComplete({
                  entry: {
                    ...entry,
                    notes: `Bound to ${entity.trim()}; ${SOUL_CONSEQUENCES.find((option) => option.value === consequence)?.label}.`,
                    acquisition: {
                      ...entry.acquisition,
                      trait: {
                        soulBound: {
                          entity: entity.trim(),
                          consequence: consequence as
                            | "insanity"
                            | "blindness"
                            | "characteristic"
                            | "mutation",
                          ...(characteristic ? { characteristic: characteristic as "ws" } : {}),
                          ...(rollRequired ? { rolledValue: roll } : {}),
                          ...(mutationName.trim() ? { mutationName: mutationName.trim() } : {}),
                        },
                      },
                    },
                  },
  ...
  if (trait.acquisition === "sanctioned-psyker") {
    ...
    const rollValid =
      !sanctionRange ||
      (Number.isInteger(roll) && roll >= sanctionRange.min && roll <= sanctionRange.max);
    const canSubmit =
      !!selectedSanction && Number.isInteger(age) && age >= 3 && age <= 30 && rollValid;
    return (
      <PickerModal
        title="Sanctioned Psyker Acquisition"
        query=""
        onQueryChange={() => undefined}
        onClose={onClose}
        hideSearch
      ...
      >
        <PickerBody>
          <div className="space-y-3">
            <PickerField
              id="sanctioning-result"
              label="Sanctioning side effect"
              value={selectedSanction ? `${selectedSanction.roll} — ${selectedSanction.name}` : ""}
              placeholder="Choose result…"
              required
              onClick={() => setPicker("sanctioning")}
            />
            {sanctionRange && (
              <div>
            ...
            <div>
              ...
              <input
                ...
                value={ageIncrease}
                onChange={(event) =>
                  setAgeIncrease(sanitizePositiveIntegerInput(event.target.value))
                }
                placeholder="3–30"
                className={`${editableInputClass(true)} mt-0.5`}
              />
            ...
            <Button
              ...
              onClick={() => {
                ...
                const sanctioning: SanctioningAcquisition = {
                  resultId: selectedSanction.id,
                  resultName: selectedSanction.name,
                  ageIncrease: age,
                  ...(sanctionRange && sanctionResultId === "reconstructed-skull"
                    ? { thronesGained: roll }
                    : sanctionRange
                      ? { rolledValue: roll }
                      : {}),
                };
                const senseRef = CYBERNETICS_REFERENCE.find(
                  (item) => item.id === "cr-cybernetic-senses"
                );
                const addedSense =
                  sanctionResultId === "optical-rupture" && senseRef
                    ? [
                        {
                          id: crypto.randomUUID(),
                          referenceId: senseRef.id,
                          name: senseRef.name,
                          craftsmanship: "Common" as const,
                          value: senseRef.value,
                          availability: senseRef.availability,
                          source: senseRef.source,
                          notes: "Eyes replaced through Sanctioned Psyker (Optical Rupture).",
                          grantedByTalentEntryUid: entry.uid,
                          grantedByTalentName: "Sanctioned Psyker",
                          grantedByType: "Trait" as const,
                        },
                      ]
                    : [];
                const gearRefId =
                  sanctionResultId === "dental-probes"
                    ? "cr-carven-dentures"
                    : sanctionResultId === "throne-wed"
                      ? "cr-chattallium-ring"
                      : null;
                const gearRef = gearRefId
                  ? GEAR_REFERENCE.find((item) => item.id === gearRefId)
                  : undefined;
                const addedGear = gearRef
                  ? [
                      {
                        id: crypto.randomUUID(),
                        referenceId: gearRef.id,
                        name: gearRef.name,
                        description: gearRef.description,
                        weight: gearRef.weight,
                        value: gearRef.value,
                        availability: gearRef.availability,
                        source: gearRef.source,
                        grantedByTalentEntryUid: entry.uid,
                        grantedByTalentName: "Sanctioned Psyker",
                        grantedByType: "Trait" as const,
                      },
                ...
                onComplete({
                  entry: {
                    ...entry,
                    notes: notesForSanctioning(sanctioning),
                    acquisition: { ...entry.acquisition, trait: { sanctioning } },
                  },
                  cybernetics: [
  ...
  return (
    <PickerModal
      title={`Skin of Iron — Rank ${skinRank}`}
      query=""
      onQueryChange={() => undefined}
      onClose={onClose}
      hideSearch
      ...
      <PickerBody>
        <div className="space-y-3">
          {ownedTraitEntries.length === 0 ? (
            <p className={uiTextBody}>
              Choose the Common-Quality cybernetic gained when this Trait is acquired.
            </p>
          ) : (
            <PickerField
              id="skin-kind"
              label={`Rank ${skinRank} benefit`}
              value={
                skinKind === "new"
                  ? "Gain a new Common cybernetic"
                  : "Upgrade an existing cybernetic to Good"
              }
              placeholder="Choose benefit…"
              required
              onClick={() => setPicker("skin-kind")}
            />
          )}
          {skinKind === "new" ? (
            <>
              <PickerField
                ...
                placeholder="Choose cybernetic…"
                required
                onClick={() => setPicker("skin-cybernetic")}
              />
              {selectedCybernetic?.requiresLocation && (
                <PickerField
                  id="skin-location"
                  label="Body location"
                  value={
                  ...
                  required
                  onClick={() => setPicker("skin-location")}
                />
              )}
            </>
          ) : (
            <>
              <PickerField
                id="skin-upgrade"
                label="Cybernetic to upgrade"
                value={
                  selectedUpgrade
                    ? `${selectedUpgrade.name} (${selectedUpgrade.craftsmanship})`
                    : ""
                }
                placeholder="Choose installed cybernetic…"
                required
                onClick={() => setPicker("skin-upgrade")}
            ...
            onClick={() => {
              if (skinKind === "upgrade") {
                ...
                onComplete({
                  entry: {
                    ...entry,
                    notes: `${selectedUpgrade.name} upgraded from ${selectedUpgrade.craftsmanship} to Good.`,
                    acquisition: {
                      ...entry.acquisition,
                      trait: {
                        skinOfIronGrants: [
                          {
                            rank: skinRank,
                            kind: "upgrade",
                            cyberneticId: selectedUpgrade.id,
                            cyberneticReferenceId: selectedUpgrade.referenceId,
                  ...
                  cybernetics: cybernetics.map((item) =>
                    item.id === selectedUpgrade.id ? { ...item, craftsmanship: "Good" } : item
                  ),
                });
                return;
              }
              ...
              const item: CyberneticItem = {
                ...
                ...(skinLocation ? { bodyLocation: [skinLocation] } : {}),
                grantedByTalentEntryUid: entry.uid,
              ...
              onComplete({
                entry: {
                  ...entry,
                  notes: `Common ${selectedCybernetic.name} gained at Rank ${skinRank}.`,
                  acquisition: {
                    ...entry.acquisition,
                    trait: {
                      skinOfIronGrants: [
                        {
                          ...
                          cyberneticReferenceId: selectedCybernetic.id,
                        },
                      ],
                    },
                  },
                },
                cybernetics: [...cybernetics, item],
              });
            }}
          >
            Apply and add Trait
