...
export function ConcealedWeaponBionicInstaller({
  ...
  onInstall: (armId: string, weapon: WeaponChoice) => void;
  onClose: () => void;
}) {
  ...
  const weapons: WeaponChoice[] = [
    ...rangedWeapons
      .filter((weapon) => weapon.class?.toLowerCase().includes("pistol") && !weapon.concealedBionic)
  ...
  return (
    <ModalShell
      ariaLabel="Install Concealed Weapon Bionic"
      onClose={onClose}
      ...
      <div className="p-4 lg:p-5 space-y-3">
        ...
        {selectingArm
          ? arms.map((item) => (
              <div key={item.id} className="space-y-2">
                <ImplantRow
                  ...
                  onRemove={() => {}}
                />
                <Button size="sm" onClick={() => setArmId(item.id)}>
                  Select Bionic Arm
                </Button>
              </div>
            ))
          : weapons.map((item) => {
              const ranged =
                item.type === "ranged"
                  ? rangedWeapons.find((weapon) => weapon.id === item.id)
                  : undefined;
              const melee =
                item.type === "melee"
                  ? meleeWeapons.find((weapon) => weapon.id === item.id)
                  : undefined;
              return (
                <div key={item.id} className="space-y-2">
                  {ranged && (
                  ...
                  <Button size="sm" onClick={() => onInstall(armId!, item)}>
                    Select Weapon
                  </Button>
                </div>
              );
