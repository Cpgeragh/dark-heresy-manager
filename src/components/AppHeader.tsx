...
export function AppHeader({ isDM, currentPath, onOpenMessages, onPlayerInstall }: AppHeaderProps) {
  ...
  const isOnDashboard = !!useMatch(isDM ? ROUTES.DM_DASHBOARD : ROUTES.PLAYER_DASHBOARD);
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-800 bg-slate-900/80 backdrop-blur">
      <div className="max-w-5xl mx-auto px-4 py-2 grid grid-cols-[1fr_auto_1fr] items-center">
        {/* Left icons — home/back + messages (players only) */}
        <div className="flex items-center gap-2">
          ...
          {!isDM && onOpenMessages && isOnCharacterSheet && (
            <button
